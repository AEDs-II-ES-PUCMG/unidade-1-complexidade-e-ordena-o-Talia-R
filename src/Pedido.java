...
public class Pedido implements Comparable<Pedido>{
	...
	private double aplicarDesconto(double preco){
		return preco*DESCONTO_PG_A_VISTA;
	}
	...
	/** Indica a forma de pagamento do pedido sendo: 1, pagamento à vista; 2, pagamento parcelado */
		...
		for (int i = 0; i < itens.length; i++) {
			valorAtual = itens[i].getPrecoVenda();
			if(formaDePagamento == 1){
				valorAtual -= aplicarDesconto(valorAtual);
			}
			valorPedido += valorAtual;
		}
		return valorPedido;
	}
	...
	public double precoSemRepeticao(){
		double soma = 0;
		for (int i = 0; i < itens.length; i++) {
			soma += itens[i].getPrecoVenda();
		}
	...
	public double valorMedio(){
		return precoSemRepeticao()/itens.length;
	}
